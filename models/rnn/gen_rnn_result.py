...
if __name__=='__main__':
    checkpoint_path=os.path.join('checkpoints','rnn_model')
    ...
    X_train,X_test=X_train.reshape(-1,51,6),X_test.reshape(-1,51,6)

    inputs=torch.from_numpy(np.array(X_test[:4]).astype(np.float32)).to(device)
    preds=predictor.predict(inputs)

    targets=yscaler.inverse_transform(np.array(y_test[:4]))
